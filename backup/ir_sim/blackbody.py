...
def integration(T, lambd=(sp.lambd1, sp.lambd2), Phi=(sp.Phi_r, sp.Phi_s), A_sens=sp.A_sens, Omega=sp.Omega):
    """
    Calculates the IR power landing on the sensor of a camera observing a black body radiator.
    ...
    lambd: (float, float), optional
        Lower and upper bounds of the wavelength range of interest.
    
    Phi: (float, float), optional
        Angles between a normal to the receiver surface and line connecting receiver and radiator centers.
    
    A_sens: float, optional
        Sensitive area of one pixel
    ...
    -------
    P: array
        IR power impinged on a pixel in the middle of the sensor at given temperatures.
    """
...
def power_distribution_over_sens_area(P, size=(sp.pix_v, sp.pix_h), fl=sp.fl, pitch = sp.pitch):
    """
    Calculates the power distribution over the sensor area due to the cosine-fourth-power law of illumination.
    
    Parameters
    ----------
    P: array
        IR power impinged on the central pixel of the sensor.
    
    size: (int, int), optional;
        Vertical and horizontal resolution of the sensor.
    ...
    -------
    pd: array
        IR power impinged on each of the detector pixels at all input power values.
    """
    ...
    # Define array for saving power distribution
    # calculated values
